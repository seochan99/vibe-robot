...
In this paper, \textbf{VibeRobot!} denotes the end-to-end interactive system, while \textbf{ArmCraft} denotes the interaction architecture inside that system.

\begin{figure*}[t]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/framework_tikz}}
...
\begin{table}[t]
  \caption{Failure taxonomy and default patch families in ArmCraft.}
  \label{tab:failure-taxonomy}
  \begin{tabular}{p{2.2cm}p{4.6cm}}
    \toprule
    Failure type & Default patch family \\
    \midrule
    Collision & waypoint offset, path replan, speed cap \\
    Grasp fail & top-down regrasp, gripper width/force tuning \\
    Unreachable/IK & target retreat, pose relaxation, alternate approach angle \\
    Workspace violation & bounds clamp, intermediate waypoint insertion \\
    Perception lost & target-lock refresh, re-detect wait, conservative fallback \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Runtime UI Walkthrough}
To ground the architecture in observed interaction states, Figures~\ref{fig:ui-flow-a} and~\ref{fig:ui-flow-b} capture the runtime sequence from landing to approval-gated execution. The walkthrough highlights three properties that were repeatedly requested during QA: (1) no actuation before explicit approval, (2) visible separation between chat preview and right-panel execution, and (3) bounded failure-repair iterations that preserve conversational context.

\begin{figure*}[t]
  \centering
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{cc}
    \includegraphics[width=0.49\textwidth]{figures/0.onboarding-screen.png} &
    \includegraphics[width=0.49\textwidth]{figures/1.chat-to-message-pick-up-the-apple.png} \\
    \includegraphics[width=0.49\textwidth]{figures/2.reasoning.png} &
    \includegraphics[width=0.49\textwidth]{figures/5.approve-and-work.png}
  \end{tabular}
  \caption{Runtime walkthrough (Part I, left-to-right/top-to-bottom): S0 onboarding and scene setup, S1 command entry, S2 reasoning + simulation preview, and S3 explicit approval gate (\textit{Reject} vs \textit{Approve \& Execute}).}
  \Description{Four enlarged interface screenshots showing setup, command entry, preview generation, and approval gating in the VibeRobot runtime flow.}
  \label{fig:ui-flow-a}
\end{figure*}
...
\begin{figure*}[t]
  ...
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{cc}
    \includegraphics[width=0.49\textwidth]{figures/3.simulator-1-pick-apple.png} &
    \includegraphics[width=0.49\textwidth]{figures/4.simulator-1-pick-up-apple.png}
  \end{tabular}
  \caption{Runtime walkthrough (Part II): S4 approved execution start and S5 live execution status in the simulator panel.}
  \Description{Two enlarged interface screenshots showing post-approval execution in the VibeRobot simulator panel.}
  \label{fig:ui-flow-b}
\end{figure*}
...
\begin{figure}[t]
  \centering
  \resizebox{\columnwidth}{!}{\input{figures/study_design_tikz}}
  \caption{Planned CHI evaluation: Study 1 uses a 2x2 component ablation under fixed sim-first approval; Study 2 performs think-aloud and interview-based mechanism tracing.}
  \Description{Diagram of Study 1 with four conditions from failure-card and patch factors, and Study 2 qualitative follow-up.}
  \label{fig:study-design}
\end{figure}

\subsection{Study 1: Controlled Experiment (N=32--40)}
\textbf{Design.}
We use a 2x2 component ablation with \emph{sim-first preview + explicit approval} fixed in every condition.
\begin{itemize}
  \item \textbf{Factor F1}: Failure Inspector card (\textit{Off}/\textit{On})
  \item \textbf{Factor F2}: Patch Suggestions (\textit{Off}/\textit{On})
\end{itemize}

\textbf{Conditions.}
\begin{itemize}
  \item \textbf{C0 (Base)}: sim-first + approval, no failure card, no patches.
  \item \textbf{C1 (+Card)}: C0 + failure card only.
  \item \textbf{C2 (+Patch)}: C0 + patch suggestions only.
  \item \textbf{C3 (ArmCraft Full)}: failure card + patch suggestions.
\end{itemize}

\textbf{Tasks.}
We use tasks that directly probe known HCI bottlenecks in robot vibe coding:
\begin{itemize}
  \item \textbf{T1: Manipulation baseline} (pick-and-place, sorting, reach-avoid) with perturbation trials.
  \item \textbf{T2: Deictic correction chain} (e.g., ``not that one, the next one'') to stress context carry-over.
  \item \textbf{T3: Mid-task requirement change} (e.g., new exclusion constraints after first attempt).
  \item \textbf{T4: Preview-boundary judgment} where users must decide reject/approve from previewed risk.
\end{itemize}

\begin{table}[t]
  \caption{Condition features in Study 1 (2x2 ablation).}
  \label{tab:conditions}
  \centering
  \scriptsize
  \begin{tabular}{p{2.35cm}cccc}
    \toprule
    Feature & C0 Base & C1 +Card & C2 +Patch & C3 Full \\
    \midrule
    Sim preview before execute & Yes & Yes & Yes & Yes \\
    Explicit approval gate & Yes & Yes & Yes & Yes \\
    Failure card diagnosis & No & Yes & No & Yes \\
    Patch suggestions (2--3) & No & No & Yes & Yes \\
    Retry context lock & No & Yes & Yes & Yes \\
    \bottomrule
  \end{tabular}
  \normalsize
\end{table}
...
\textbf{Recovery trajectory metrics (signature)}:
\begin{itemize}
  \item \textbf{Recovery depth}: number of repair turns from first failure to first success.
  \item \textbf{Patch yield}: fraction of applied patches that reduce failure severity or produce success in the next run.
  \item \textbf{Failure-transition profile}: transition matrix over the five failure types in Table~\ref{tab:failure-taxonomy}.
  \item \textbf{Attribution calibration curve}: retry-wise 100-point allocation over self/robot/AI and its convergence pattern.
\end{itemize}

\textbf{Process traces}: event logs, stage durations, rejected vs. accepted patches, and post-patch delta in failure type.

\subsection{Analysis Plan}
For Study 1, we will fit mixed-effects models with \textbf{F1}, \textbf{F2}, and their interaction as fixed effects and participant/task as random effects, following established guidance for confirmatory repeated-measures analysis~\cite{barr2013maximal}. For non-normal outcomes, we will use generalized mixed models. We will report effect sizes and confidence intervals, not only p-values~\cite{lakens2013effectsizes}. Planned contrasts test (a) card-only gain, (b) patch-only gain, and (c) super-additive gain in the full loop.
...
Our primary evidence target is interaction quality under controlled failure-repair cycles, not hardware benchmarking. We intentionally use simulation because it supports repeatable failure injection, condition-controlled comparison, and auditable replay of recovery trajectories. Real hardware transfer remains a follow-up stage and may introduce perception/calibration failures beyond the current simulator envelope; we therefore report hardware generalization as future validation rather than conflating it with interaction-mechanism evidence.
