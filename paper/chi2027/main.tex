...
\begin{figure*}[t]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/ui_execution_flow_tikz}}
  \caption{End-to-end UI execution flow reconstructed from representative prototype screenshots: landing/setup, command entry, pipeline + preview, approval gate, approved execution, and failure-aware retry.}
  \Description{Six-panel runtime walkthrough showing state transitions and gating logic in the VibeRobot interface, including preview-only mode before approval and execution mode after approval.}
  \label{fig:ui-flow}
\end{figure*}
