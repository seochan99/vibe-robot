...
class VibeRobotPipeline:
    ...
    def approve_and_execute(self, result: PipelineResult) -> PipelineResult:
        """Execute a plan that was awaiting approval."""
    ...
    def _run_simulation_preview(
        ...
        """Run the plan in simulation for preview (non-destructive)."""
        # Save current state
        ...
        try:
            ...
            # Capture final frame
            try:
                frame = self.env.render_offscreen()
                frames.append(frame)
            except Exception:
                pass
    ...
    def _get_scene_summary(self) -> str:
        """Get a brief scene summary for intent inference context."""
        if not self._scene_objects:
            return "Empty scene with Franka Panda robot arm on desk"
        names = [obj.name for obj in self._scene_objects if not obj.properties.get("fixed")]
        return f"Desk scene with objects: {', '.join(names)}"
